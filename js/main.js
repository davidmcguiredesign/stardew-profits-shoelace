...
const chart = d3.select( "div.graph" )
	.append( "svg" )
	.attr( "width", svgWidth )
...
const gAxis = chart.append( "g" );
const gTitle = chart.append( "g" );
const gProfit = chart.append( "g" );
const gSeedLoss = chart.append( "g" );
const gFertLoss = chart.append( "g" );
const gIcons = chart.append( "g" );
const gTooltips = chart.append( "g" );
...
function renderGraph() {
	...
	if ( width < svgMinWidth )
		width = svgMinWidth;
	chart.attr( "width", width ).style( "padding-top", "12px" );
	...
	barsProfit = gProfit.selectAll( "rect" )
		...
		.enter()
		.append( "rect" )
		.attr( 'class', d => d.drawProfit >= 0 ? 'profit bar' : 'profit bar bad' )
		.attr( "x", function ( d, i ) {
	...
	barsTooltips = gTooltips.selectAll( "rect" )
		...
		.attr( "height", function ( d ) {
			var topHeight = barWidth + barPadding;

			if ( d.drawProfit >= 0 )
				topHeight += height - y( d.drawProfit );
			...
			lossArray = lossArray.sort( ( a, b ) => a - b );
		...
		.on( "mouseover", function ( d ) {
			...
			infoPanel.append( "h3" ).attr( "class", "tooltipTitle" ).text( d.name );

			var tooltipTable = infoPanel.append( "table" )
			...
			if ( options.extra ) {
				...
				infoPanel.append( "h3" ).attr( "class", "tooltipTitleExtra" ).text( "Crop info" );
				tooltipTable = infoPanel.append( "table" )
					.attr( "class", "tooltipTable" )
					.attr( "cellspacing", 0 );
		...
		.on( "mousemove", function () {
			infoPanel.style( "top", ( d3.event.pageY - 16 ) + "px" ).style( "left", ( d3.event.pageX + 20 ) + "px" );
		} )
		.on( "mouseout", function () { infoPanel.style( "visibility", "hidden" ); } )
		.on( "click", function ( d ) {
